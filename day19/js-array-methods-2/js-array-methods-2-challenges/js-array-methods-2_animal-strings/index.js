...
const catStartingFromIndexFiveExists = animalStrings.find((cat) =>
  cat.startsWith("c")
);
console.log(catStartingFromIndexFiveExists);

// Hint: Besides the array method, check out the string method `startsWith()`.
const firstAnimalStartingWithLetterP = animalStrings.find((randomAnimal) =>
  randomAnimal.startsWith("p")
);
console.log(firstAnimalStartingWithLetterP);

const indexOfGiraffe = animalStrings.findIndex((giraffe) =>
  giraffe.startsWith("g")
);
...
const anyAnimalEndsWithLetterZ = animalStrings.find((randomAnimal) =>
...
const everyAnimalHasMoreThanTwoLetters = animalStrings.every(
  (randomAnimal) => randomAnimal.length > 2
);
